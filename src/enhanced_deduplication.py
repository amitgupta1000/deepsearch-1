...
async def llm_intelligent_deduplicate(content: str, report_type: str = "detailed") -> str:
    ...
    # Check if LLM deduplication should be applied
    if LLM_DEDUP_DETAILED_ONLY and report_type != "detailed":
        logging.info(f"LLM deduplication disabled for {report_type} reports")
        return content
...
"""
    ...
    try:
        ...
        if response and hasattr(response, 'content'):
            ...
            # Validate that the result isn't too short (potential over-deduplication)
...
async def enhanced_deduplicate_content(text: str, report_type: str = "detailed") -> str:
    ...
    Args:
        ...
        report_type: Type of report ("detailed" or "concise")
    ...
    if USE_LLM_DEDUPLICATION and basic_words >= LLM_DEDUP_MIN_WORDS:
        final_result = await llm_intelligent_deduplicate(basic_result, report_type)
